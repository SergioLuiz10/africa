...
import dijaraImage from "../assets/choro.jpg";
...
const DijaraSection: React.FC<Dijara> = ({ language }) => {
  ...
  return (
    <section className="bg-white py-20" id="dijara">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-[#78350f] mb-4">
            {t.title}
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            {t.paragraphs.map((text, index) => (
              <div
                key={index}
                className="bg-[#78350f] text-white p-6 rounded-xl shadow-md"
              >
                <p>{text}</p>
              </div>
            ))}
